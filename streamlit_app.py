...
with tabs[3]:
    ...
    with col_rules1:
        ...
        st.markdown('<div class="rule-item"><b>Respecte al silenci:</b> Les zones de cabines exigeixen silenci absolut.</div>', unsafe_allow_html=True)
    with col_rules2:
        st.markdown("#### 🍎 Alimentació i Higiene")
        st.markdown('<div class="rule-item"><b>Zones de menjar:</b> S\'ha d\'utilitzar exclusivament la sala de menjador.</div>', unsafe_allow_html=True)
...
# --- TAB RESERVA (CON VALIDACIÓN DE EMAIL Y 9 CIFRAS PARA TELÉFONO) ---
with tabs[6]:
    st.markdown('<div class="ec-card">', unsafe_allow_html=True)
    ...
    with st.form("b_form"):
        ...
        p_v = col_b.text_input(t["phone"]) # El label indica "(9 xifres)"
        ...
        if st.form_submit_button(t["booking_confirm"], use_container_width=True):
            if not n_v.strip() or not s_v.strip() or not m_v.strip() or not p_v.strip():
                ...
                st.error(t["email_error"])
            # VALIDACIÓN: Debe ser numérico Y tener exactamente 9 dígitos
